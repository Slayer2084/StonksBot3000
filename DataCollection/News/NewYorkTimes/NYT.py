...
class NYTSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for query in queries:
            ...
            yield scrapy.Request(url, method="POST", body=body, headers=headers, meta=dict(query=query))

    def parse(self, response):
        query = response.query
        print(query)
        yield scrapy.Request(url, method="POST", body=payload, headers=headers, meta=dict(query=query))
        ...
        for r in response.xpath('//*[@class="result_container"]'):
            item = {
                ...
                'Phones': ', '.join([t.replace('\xa0', ' ') for t in
                                     r.xpath('normalize-space(./*[@class="col1"]//*[@class="tel"])').getall()]),
